            ...
            <div class="page-section">
               ...
               <section id="base-style" class="card">
                  <div class="card-body">
                     <form  id="collegeUsersForm" autocomplete="off">
                        {{ csrf_field() }}
                        <fieldset>
                           <legend>College Degree Admin Management Area</legend>
               ...
               <section class="card card-fluid" id="table-show">
                  ...
                  <div class="card-body">
                     ...
                     <div class="text-muted"> Showing {{ $degAdmins->firstItem() }} to {{ $degAdmins->lastItem() }} of {{$degAdmins->total()}} entries </div>
                     <div class="table-responsive">
                        <table class="table">
                           @if(count($degAdmins) > 0 )
                           <thead>
                              <tr>
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td class="align-middle"> 
                                      @if($user->status === 0)
                                        <button data-id= "{{ $user->degree_admin_id }}" class="btn btn-primary btn-sm btn-inactive">Inactive</button>
                                          
                                          @else 
                                          <button data-id= "{{ $user->degree_admin_id }}" class="btn btn-success  btn-sm btn-active">&nbsp;Active&nbsp;&nbsp;</button>
                                      @endif

                                 </td>
                                 <td class="align-middle text-right">
                                    <button type="button" href="#edit<?php echo $user->degree_admin_id;?>" class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#edit<?php echo $user->degree_admin_id;?>">
                                       <i class="fa fa-pencil-alt"></i>
                                       <span class="sr-only">Edit</span>
                                    </button>
                                    <button type="button" href="#delete<?php echo $user->degree_admin_id;?>" class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#delete<?php echo $user->degree_admin_id;?>">
                                       <i class="far fa-trash-alt"></i>
                                       <span class="sr-only">Remove</span>
                                    </button>
                                 </td>
                              </tr>
                              <div class="modal fade" id="delete<?php echo $user->degree_admin_id;?>">
                                    ...
                                    <div class="modal-content">
                                       ...
                                       <div class="modal-footer">
                                          <div style="margin-top: 20px !important;">
                                             <a  href="{{route('sessiondel',['id'=>$user->degree_admin_id])}}" data-id="<?php echo $user->degree_admin_id;?>" class="btn btn-sm btn-outline-primary  btn-del">Yes <i class="far fa fa-check" aria-hidden="true"></i>
                                             </a>
                              ...
                              <div class="modal" id="edit<?php echo $user->degree_admin_id;?>">
                               <div class="modal-dialog">
                                <div class="modal-content">
                               ...
                               <!-- Modal body -->
                               <div class="modal-body">
                                  <form action="{{ route('sessionupdate',['id'=>$user->degree_admin_id])}}" method="POST">
                                    {{ csrf_field() }}
...
<script>
  ...
  var base_url  = '{{ URL::to("/") }}/';
   $('#BtnSubmit').on('click',function(){
      $("form#collegeUsersForm").validate({
         ...
         submitHandler:function(form, e){
            ...
            var url="<?php echo route('degreeadmin');?>";
            var cur_url="<?php echo route('degreeadmin');?>";
           axios({
            method: 'POST',
            url: url,
            data: {
         ...
         .catch(function(err){
            setTimeout(()=>{toastr.warning("Degree Admin Can not be created username already exists");
         },1000);
            img.setAttribute("src","");
            btn.style.display="block";
            loader.style.display="none";
   ...
   $("button.btn-inactive").click(function(event) {
        ...
        var id = $(this).data('id');
        var url = "/special/degadmininactive/";
        ...
        .then(function(res){
         ...
         .catch(function(err){
            setTimeout(()=>{toastr.warning("Degree Admin status could not been changed.");
         },1000);
            $('body').load(document.URL+'#collegeUsersForm')});
   ...
   $("button.btn-active").click(function(event) {
        ...
        var id = $(this).data('id');
        var url = "/special/degadminactive/";
