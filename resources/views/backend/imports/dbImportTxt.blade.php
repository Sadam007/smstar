...
@section('page-title')
<title> Exam Portal | DB Synchronization | Database Import Synchronization</title>
@endsection
...
<section id="base-style" class="card">
  <div class="card-body">
   <form  id="uploadCsvForm" autocomplete="off">
    {{ csrf_field() }}
    <fieldset>
     <legend>Database Import Synchronization</legend>
     <div class="form-group">
      <label>Upload File</label>
      <div class="custom-file">
       <input type="file" name="txt_import" id="txt_import" class="custom-file-input" id="tf3">
       <label class="custom-file-label persist" for="tf3">Choose file</label>
   </div>
</div>
<div class="form-group btn-toggle">
  <button type="submit" id="BtnSubmit" class="btn btn-primary" style="letter-spacing: 2px;"><i class="fas fa-sync-alt"></i> Synchronize </button>
</div>   
</fieldset>
<div id="loader"></div>
</form>
...
invalidHandler:function(form,validator){
    ...
    txt_import:{
     ...
     extension:"File type must be (*.txt)"
 }
},
submitHandler:function(form,e){
    ...
    var url="<?php echo route('database.import-txt-Process');?>";
    var cur_url="<?php echo route('database.import-txt');?>";
    var data=new FormData();
    data.append('txt_import',document.getElementById('txt_import').files[0]);
