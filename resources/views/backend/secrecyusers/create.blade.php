...
@section('page-title')
<title> Exam Portal | Secrecy Management | Create Secrecy Users</title>
@endsection
         ...
         <header class="page-title-bar">
            ...
            <div class="page-section">
               ...
               <section id="base-style" class="card">
                  <div class="card-body">
                     <form  id="secrecyUsersForm" autocomplete="off">
                        {{ csrf_field() }}
                        <fieldset>
                           <legend>Secrecy Management Area</legend>
               ...
               <section class="card card-fluid" id="table-show">
                  <header class="card-header">
                     <legend>All Registered Secrecy Users</legend>
                  </header>
                  <div class="card-body">
                     <div class="form-group">
                     ...
                     <div class="text-muted"> Showing {{ $secrecyusers->firstItem() }} to {{ $secrecyusers->lastItem() }} of {{$secrecyusers->total()}} entries </div>
                     <div class="table-responsive">
                        <table class="table">
                           @if(count($secrecyusers) > 0 )
                           <thead>
                              <tr>
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td class="align-middle"> 
                                      @if($user->status === 0)
                                        <button data-id= "{{ $user->sec_user_id }}" class="btn btn-primary btn-sm btn-inactive">Inactive</button>
                                          
                                          @else 
                                          <button data-id= "{{ $user->sec_user_id }}" class="btn btn-success  btn-sm btn-active">&nbsp;Active&nbsp;&nbsp;</button>
                                      @endif

                                 </td>
                                 <td class="align-middle text-right">
                                    <button type="button" href="#edit<?php echo $user->sec_user_id;?>" class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#edit<?php echo $user->sec_user_id;?>">
                                       <i class="fa fa-pencil-alt"></i>
                                       <span class="sr-only">Edit</span>
                                    </button>
                                    <button type="button" href="#delete<?php echo $user->sec_user_id;?>" class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#delete<?php echo $user->sec_user_id;?>">
                                       <i class="far fa-trash-alt"></i>
                                       <span class="sr-only">Remove</span>
                                    </button>
                                 </td>
                              </tr>
                              <div class="modal fade" id="delete<?php echo $user->sec_user_id;?>">
                                    ...
                                    <div class="modal-content">
                                       ...
                                       <div class="modal-footer">
                                          <div style="margin-top: 20px !important;">
                                             <a  href="{{route('sessiondel',['id'=>$user->sec_user_id])}}" data-id="<?php echo $user->sec_user_id;?>" class="btn btn-sm btn-outline-primary  btn-del">Yes <i class="far fa fa-check" aria-hidden="true"></i>
                                             </a>
                              ...
                              <div class="modal" id="edit<?php echo $user->sec_user_id;?>">
                               <div class="modal-dialog">
                                <div class="modal-content">
                                 <div class="modal-header">
                                  <h4 class="modal-title">Update Session : {{$user->sec_user_id}}</h4>
                                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                               </div>

                               <!-- Modal body -->
                               <div class="modal-body">
                                  <form action="{{ route('sessionupdate',['id'=>$user->sec_user_id])}}" method="POST">
                                    {{ csrf_field() }}
                                 
                                     <div class="form-group">
                                      <label for="tf1">Session Code</label>
                                      <input type="number" name="sessionCode" class="form-control" id="tf1" aria-describedby="tf1Help" value="{{$user->sec_user_id}}">
                                    </div>
                                    <div class="form-group">
                                      <label for="tf1">Session</label>
                                      <input type="text" name="session" class="form-control" id="tf1" aria-describedby="tf1Help" value="{{$user->sec_user_id}}">
                                    </div>
                                    <div class="form-group">
                                      <label for="tf1">Status</label>
                                      <input type="number" name="status" class="form-control" id="tf1" aria-describedby="tf1Help" value="{{$user->sec_user_id}}">
                                    </div>
                                    <div class="form-group">
...
<script>
  ...
  var base_url  = '{{ URL::to("/") }}/';
   $('#BtnSubmit').on('click',function(){
      $("form#secrecyUsersForm").validate({
         errorElement:'span',
         ...
         submitHandler:function(form, e){
            ...
            var url="<?php echo route('secrecyuser.store');?>";
            var cur_url="<?php echo route('secrecyuser.create');?>";
           axios({
            method: 'POST',
            url: url,
            data: {
         ...
         .then(function(res){
            ...
            if(good==true){
               ...
               $('body').load(document.URL+'#secrecyUsersForm');
            }
         })
         .catch(function(err){
            setTimeout(()=>{toastr.error("Secrecy User does not created username already exists");
         },1000);
            img.setAttribute("src","");
            btn.style.display="block";
            loader.style.display="none";
   ...
   $("button.btn-inactive").click(function(event) {
        ...
        var url = "/admin/secrecyuserinactive/";
        ...
        .then(function(res){
         ...
         .catch(function(err){
            setTimeout(()=>{toastr.error("User status could not been changed.");
         },1000);
            $('body').load(document.URL+'#specialUsersForm')});
   ...
   $("button.btn-active").click(function(event) {
        ...
        var id = $(this).data('id');
        var url = "/admin/secrecyuseractive/";
