...
@section('page-title')
<title> Exam Portal | Student Management | Add Certificate</title>
@endsection
 ...
 <section id="base-style" class="card">
  <div class="card-body">
   <form  id="certificateForm" autocomplete="off">
    {{ csrf_field() }}
    <fieldset>
      <legend>Certificate Management Area</legend>
     <div class="form-group">
      <label class="col-form-label" for="certificate">Certificate Name</label>
      <input type="text" name="certificate" class="form-control username" placeholder="Enter Certificate Name" id="certificate"> 
    </div>
...
submitHandler:function(form,e){
  ...
  var url="<?php echo route('certificate.process');?>";
  var cur_url="<?php echo route('certificate.create');?>";
  ...
  .then(function(res){
   ...
   $('body').load(document.URL+'#certificateForm');
 }
})
.catch(function(err){
  setTimeout(()=>{toastr.warning("Certificate can not be added");
},1000);
  img.setAttribute("src","");
  btn.style.display="block";
  loader.style.display="none";
