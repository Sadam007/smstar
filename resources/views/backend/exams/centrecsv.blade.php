...
@section('page-title')
<title> Exam Portal | Exams Management | Add Centre</title>
@endsection
...
<section id="base-style" class="card">
  <div class="card-body">
   <form  id="uploadCsvForm" autocomplete="off">
    {{ csrf_field() }}
    <fieldset>
     <legend>Centre Code Management Area</legend>
...
<section class="card card-fluid" id="table-show">
  <header class="card-header">
   <legend>All Registered Centers</legend>
</header>
<div class="card-body">
   <div class="form-group">
...
<td>        
    <a href="#" style="margin-left: 10px;">{{ $center->addedby}}</a>
</td>
<td class="align-middle"> {{ $center->ccode}} </td>
<td class="align-middle"> {{ $center->name_of_centre}} </td>
<td class="align-middle text-right">
    <button type="button" href="#edit<?php echo $center->center_id;?>" class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#edit<?php echo $center->center_id;?>">
     <i class="fa fa-pencil-alt"></i>
     <span class="sr-only">Edit</span>
 </button>
 <button type="button" href="#delete<?php echo $center->center_id;?>" class="btn btn-sm btn-secondary" data-toggle="modal" data-target="#delete<?php echo $center->center_id;?>">
     <i class="far fa-trash-alt"></i>
     <span class="sr-only">Remove</span>
 </button>
</td>
</tr>
<div class="modal fade" id="delete<?php echo $center->center_id;?>">
...
<div class="modal-footer">
  <div style="margin-top: 20px !important;">
   <a  href="{{route('sessiondel',['id'=>$center->center_id])}}" data-id="<?php echo $center->center_id;?>" class="btn btn-sm btn-outline-primary  btn-del">Yes <i class="far fa fa-check" aria-hidden="true"></i>
   </a>
...
<div class="modal" id="edit<?php echo $center->center_id;?>">
 <div class="modal-dialog">
    <div class="modal-content">
       <div class="modal-header">
          <h4 class="modal-title">Update Session : {{$center->name_of_centre}}</h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body">
          <form action="{{ route('sessionupdate',['id'=>$center->center_id])}}" method="POST">
            {{ csrf_field() }}
            
            <div class="form-group">
              <label for="tf1">Session Code</label>
              <input type="number" name="sessionCode" class="form-control" id="tf1" aria-describedby="tf1Help" value="{{$center->examcode}}">
          </div>
          <div class="form-group">
              <label for="tf1">Session</label>
              <input type="text" name="session" class="form-control" id="tf1" aria-describedby="tf1Help" value="{{$center->examcode}}">
          </div>
          <div class="form-group">
              <label for="tf1">Status</label>
              <input type="number" name="status" class="form-control" id="tf1" aria-describedby="tf1Help" value="{{$center->examcode}}">
          </div>
          <div class="form-group">
...
<div class="page-sidebar page-sidebar-fixed">
  ...
  <nav id="nav-content" class="nav flex-column mt-4">
   <a href="{{ route('examcsv') }}" class="nav-link smooth-scroll">All Exams</a>
...
submitHandler:function(form,e){
    ...
    var url="<?php echo route('centrecsvpost');?>";
    var cur_url="<?php echo route('centrecsv');?>";
